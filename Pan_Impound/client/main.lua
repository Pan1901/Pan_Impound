...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if isInMarker and not hasAlreadyEnteredMarker then
      ...
      TriggerEvent('Pan_Impound:hasEnteredMarker', currentZone)
    end

    if not isInMarker and hasAlreadyEnteredMarker then
      hasAlreadyEnteredMarker = false
      TriggerEvent('Pan_Impound:hasExitedMarker', LastZone)
    end
  end
end)
...
AddEventHandler('Pan_Impound:hasEnteredMarker', function(zone)
...
AddEventHandler('Pan_Impound:hasExitedMarker', function(zone)
  ESX.UI.Menu.CloseAll()
  CurrentAction = nil
end)

RegisterNetEvent("Pan_Impound:impound_nearest_vehicle")
AddEventHandler("Pan_Impound:impound_nearest_vehicle", function(args)
  local coords = GetEntityCoords(GetPlayerPed(-1))
  ...
  if DoesEntityExist(vehicle) then
    if hasImpoundAppropriateJob() then
      if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
        ...
        ESX.TriggerServerCallback('Pan_Impound:impound_vehicle', function()
...
function ListVehiclesMenu()
  ...
  ESX.TriggerServerCallback('Pan_Impound:get_vehicle_list', function(vehicles)
...
function loadListVehiclePage(elementPages, page)
  if page <= tablelength(elementPages) then
    ...
    function(data, menu)
      ...
      elseif data.current.value == "ne" then
        ...
        if Config.UserMustPayFine then
          ESX.TriggerServerCallback('Pan_Impound:check_money', function(paidFine)
            if paidFine then
  ...
  function SpawnVehicle(vehicle)
    ...
    ESX.TriggerServerCallback('Pan_Impound:retrieve_vehicle', function()
