...
RegisterCommand('impound', function(source, args)
  TriggerClientEvent('Pan_Impound:impound_nearest_vehicle', source)
end)

ESX.RegisterServerCallback('Pan_Impound:impound_vehicle', function(source, cb, plate)
  ImpoundVehicle(plate)
  cb()
end)

ESX.RegisterServerCallback('Pan_Impound:retrieve_vehicle', function(source, cb, plate)
  RetrieveVehicle(plate)
  cb()
end)

ESX.RegisterServerCallback('Pan_Impound:get_vehicle_list', function(source, cb)
  ...
  MySQL.Async.fetchAll("SELECT * FROM impounded_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.getIdentifier()}, function(data)
    for _,v in pairs(data) do
      ...
      table.insert(vehicles, {vehicle = vehicle, pound = v.pound, can_release = VehicleEligableForRelease(v)})
    end
    cb(vehicles)
  end)
end)

ESX.RegisterServerCallback('Pan_Impound:check_money', function(source, cb)
...
function ProcessImpoundment(plate, current_time, vehicles)
  for index, vehicle in pairs(vehicles) do
    ...
    MySQL.Async.execute("DELETE FROM owned_vehicles WHERE plate=@plate LIMIT 1", 
      {
        ['@plate'] = plate
      }
    )
...
function RetrieveVehicle(plate)
  ...
  MySQL.Async.fetchAll('SELECT * FROM impounded_vehicles WHERE plate = @plate LIMIT 1', {
    ...
    for index, vehicle in pairs(vehicles) do
      ...
      if Config.OwnedVehiclesHasPlateColumn then
        MySQL.Async.execute("INSERT INTO `owned_vehicles` (`plate`, `vehicle`, `owner`, `pound`) VALUES(@plate, @vehicle, @owner, '0')",
          {
            ['@plate'] = plate,
            ['@vehicle'] = vehicle.vehicle,
            ['@owner'] = vehicle.owner
        ...
        MySQL.Async.execute("INSERT INTO `owned_vehicles` (`vehicle`, `owner`, `pound`) VALUES(@vehicle, @owner, '0')",
          {
            ['@vehicle'] = vehicle.vehicle,
            ['@owner'] = vehicle.owner
          }
